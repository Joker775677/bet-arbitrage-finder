...
async function scrapeWinlineDom() {
  ...
  try {
    ...
    // first so the cards include "�������/������" instead of only live rows.
    ...
    const payload = await page.evaluate(() => {
      ...
      for (const a of eventAnchors) {
        ...
        while (node && node.parentElement) {
          ...
          if (
            /(?:�������|������|\d{2}\.\d{2})\s+\d{1,2}:\d{2}/i.test(candidateText) &&
        ...
        const timeText = flat.find((x) => /\d{1,2}:\d{2}/.test(x))
          || cardText.match(/(�������|������|\d{2}\.\d{2})\s+\d{1,2}:\d{2}/i)?.[0]
          || null;
        const isLive = /(?:^|\s)(?:1�|2�|3�|\d{1,2}'(?:\+\d+)?|Tx\d+)/i.test(cardText);
        const teams = [];
        for (const row of flat) {
          if (!row) continue;
          if (/^(�������|������|\d{2}\.\d{2})\s+\d{1,2}:\d{2}$/i.test(row)) continue;
          ...
          if (/^(����|1 ����|2 ����|1�|2�)$/i.test(row)) continue;
          teams.push(row);
